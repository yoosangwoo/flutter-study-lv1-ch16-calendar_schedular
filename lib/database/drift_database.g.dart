...
class $CategoryColorsTable extends CategoryColors
  ...
  late final GeneratedColumn<int> id = GeneratedColumn<int>(
      ...
      defaultConstraints:
          GeneratedColumn.constraintIsAlways('PRIMARY KEY AUTOINCREMENT'));
  static const VerificationMeta _haxCodeMeta =
      const VerificationMeta('haxCode');
  @override
  late final GeneratedColumn<String> haxCode = GeneratedColumn<String>(
      'hax_code', aliasedName, false,
      type: DriftSqlType.string, requiredDuringInsert: true);
  @override
  List<GeneratedColumn> get $columns => [id, haxCode];
  @override
  ...
  VerificationContext validateIntegrity(Insertable<CategoryColor> instance,
    ...
    if (data.containsKey('hax_code')) {
      context.handle(_haxCodeMeta,
          haxCode.isAcceptableOrUnknown(data['hax_code']!, _haxCodeMeta));
    } else if (isInserting) {
      context.missing(_haxCodeMeta);
    }
    return context;
  }
  ...
  CategoryColor map(Map<String, dynamic> data, {String? tablePrefix}) {
    ...
    return CategoryColor(
      id: attachedDatabase.typeMapping
          .read(DriftSqlType.int, data['${effectivePrefix}id'])!,
      haxCode: attachedDatabase.typeMapping
          .read(DriftSqlType.string, data['${effectivePrefix}hax_code'])!,
    );
  }
...
class CategoryColor extends DataClass implements Insertable<CategoryColor> {
  final int id;
  final String haxCode;
  const CategoryColor({required this.id, required this.haxCode});
  @override
  Map<String, Expression> toColumns(bool nullToAbsent) {
    final map = <String, Expression>{};
    map['id'] = Variable<int>(id);
    map['hax_code'] = Variable<String>(haxCode);
    return map;
  }
  ...
  factory CategoryColor.fromJson(Map<String, dynamic> json,
    ...
    return CategoryColor(
      id: serializer.fromJson<int>(json['id']),
      haxCode: serializer.fromJson<String>(json['haxCode']),
    );
  }
  @override
  Map<String, dynamic> toJson({ValueSerializer? serializer}) {
    ...
    return <String, dynamic>{
      'id': serializer.toJson<int>(id),
      'haxCode': serializer.toJson<String>(haxCode),
    };
  }

  CategoryColor copyWith({int? id, String? haxCode}) => CategoryColor(
        id: id ?? this.id,
        haxCode: haxCode ?? this.haxCode,
      );
  @override
  String toString() {
    return (StringBuffer('CategoryColor(')
          ..write('id: $id, ')
          ..write('haxCode: $haxCode')
          ..write(')'))
        .toString();
  }

  @override
  int get hashCode => Object.hash(id, haxCode);
  @override
  bool operator ==(Object other) =>
      identical(this, other) ||
      (other is CategoryColor &&
          other.id == this.id &&
          other.haxCode == this.haxCode);
}
...
class CategoryColorsCompanion extends UpdateCompanion<CategoryColor> {
  ...
  CategoryColorsCompanion.insert({
    this.id = const Value.absent(),
    required String haxCode,
  }) : hexCode = Value(haxCode);
  static Insertable<CategoryColor> custom({
    Expression<int>? id,
    Expression<String>? haxCode,
  }) {
    return RawValuesInsertable({
      if (id != null) 'id': id,
      if (haxCode != null) 'hax_code': haxCode,
    });
  }

  CategoryColorsCompanion copyWith({Value<int>? id, Value<String>? haxCode}) {
    return CategoryColorsCompanion(
      id: id ?? this.id,
      hexCode: haxCode ?? this.hexCode,
    );
  }
  ...
  Map<String, Expression> toColumns(bool nullToAbsent) {
    ...
    if (hexCode.present) {
      map['hax_code'] = Variable<String>(hexCode.value);
    }
    return map;
  }
